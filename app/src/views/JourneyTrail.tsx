...
export default function JourneyTrail({ progress }: { progress: ProgressData }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('aramit-path', sync);
    window.addEventListener('storage', sync);
    return () => {
      window.removeEventListener('aramit-path', sync);
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem('aramit_pan_hint') === '1') return;
    setShowPanHint(true);
    const t = window.setTimeout(() => {
      setShowPanHint(false);
      sessionStorage.setItem('aramit_pan_hint', '1');
    }, 4200);
  ...
  const openPlay = (s: Station) => {
    sessionStorage.setItem('aramit_focus_act', s.act.id);
