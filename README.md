...
עדכונים אחרונים באתר החי ([menavtim](https://yh-edu.github.io/menavtim/)):

### הצופן הארמי — חדר בריחה בבית המדרש

תחנת הסיום של המסע (אחרי יחידות הלימוד). נפתחת כשכל התחנות הקודמות הושלמו, או תמיד במצב **תרגול חופשי**.
...
### שיפורים במנווטים (האפליקציה הראשית)

- **החלפת דמות בלי לאפס התקדמות** — הדמות על המפה קוסמטית; מיקום המפה וההתקדמות נשמרים לפי שם+אימוג'י.
- **שמירה לפי שם + אימוג'י מזהה** — כל שחקן במכשיר מקבל קובץ התקדמות נפרד (בלי דליפה בין משתמשים); סגירת הדפדפן דורשת הרשמה מחדש לפגישה, אבל ההתקדמות נשארת ב־`localStorage`.
- **כניסה תמיד למסך הנחיתה** בפתיחה קרה (cold start) — לא קופצים ישר למפה.
- **TTS (הקראה)** — כפתור בולט בפעילויות; הקשה קצרה מכבה ומדליקה.
- **מסך מלא** — זמין בכל מסכי המשחק ובחדר הבריחה.
- **המוזאיקה הארמית** — שם משימת הצביעה בחלק האחרון של מבחן הגמר (לפני חדר הבריחה).
- **משוב** — קישור קטן במסך הפתיחה (Gmail compose) ליצירת קשר.
...
- **מפת מסע (Phaser)** — שביל ארץ־ישראל: כרם → גשר → טרסות → **בית מדרש**, וסיום ב**הצופן הארמי** (חדר בריחה).
...
14. **בחינה סופית** — כל המכניקות; אחר כך **הצופן הארמי** (חדר בריחה בבית המדרש)
