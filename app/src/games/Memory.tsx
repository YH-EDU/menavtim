...
// משחק זיכרון: שני צדדים של אותו זוג (למשל ארמית ↔ עברית).

interface Card {
  id: number;
  pair: number;
  text: string;
  rashi: boolean; // true = צד «א» (מודגש)
}
        ...
        {cards.map((card) => {
          ...
          return (
            <button
              key={card.id}
              onClick={() => flip(card)}
              className={card.rashi && isOpen ? 'phrase-font' : ''}
              style={{
                minHeight: 84,
                borderRadius: 12,
                ...
                fontSize: isOpen ? (card.text.length > 10 ? 15 : card.rashi ? 22 : 18) : 30,
